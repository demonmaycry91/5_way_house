...
import os
import json
from flask import render_template, request, flash, redirect, url_for, Blueprint, jsonify, current_app
...
def dashboard():
    ...
    for location_name in LOCATIONS:
        ...
        if business_day is None:
            status_info = {'status_text': '尚未開帳', 'message': '點擊以開始本日營業作業。', 'badge_class': 'bg-secondary', 'url': url_for('cashier.start_day', location=location_name)}
        elif business_day.status == 'OPEN':
            status_info = {'status_text': '營業中', 'message': f"本日銷售額: ${business_day.total_sales:,.0f}", 'badge_class': 'bg-success', 'url': url_for('cashier.pos', location=location_name)}
        elif business_day.status == 'PENDING_REPORT':
            status_info = {'status_text': '待確認報表', 'message': '點擊以檢視並確認本日報表。', 'badge_class': 'bg-warning text-dark', 'url': url_for('cashier.daily_report', location=location_name)}
        elif business_day.status == 'CLOSED':
            status_info = {'status_text': '已日結', 'message': '本日帳務已結算，僅供查閱。', 'badge_class': 'bg-primary', 'url': url_for('cashier.daily_report', location=location_name)}
...
def start_day(location):
    ...
    if request.method == 'POST':
        try:
            ...
            new_business_day = BusinessDay(date=today, location=location, location_notes=location_notes, status='OPEN', opening_cash=opening_cash)
            ...
            return redirect(url_for('cashier.start_day', location=location))
    return render_template('cashier/start_day_form.html', location=location, today_date=today.strftime('%Y-%m-%d'))
...
def pos(location):
    ...
    if not business_day:
        ...
        return redirect(url_for('cashier.dashboard'))
    return render_template('cashier/pos.html', location=location, today_date=today.strftime('%Y-%m-%d'), initial_sales=business_day.total_sales, initial_items=business_day.total_items, initial_transactions=business_day.total_transactions)
...
def close_day(location):
    ...
    return render_template('cashier/close_day_form.html', location=location, today_date=today.strftime('%Y-%m-%d'), denominations=denominations)
...
def daily_report(location):
    today = date.today()
    business_day = BusinessDay.query.filter(BusinessDay.date == today, BusinessDay.location == location, BusinessDay.status.in_(['PENDING_REPORT', 'CLOSED'])).first()
    if not business_day:
    ...
    return render_template('cashier/daily_report.html', day=business_day, 帳面總額=expected_total, 帳差=difference)
...
def confirm_report(location):
    ...
    today = date.today()
    business_day = BusinessDay.query.filter_by(date=today, location=location, status='PENDING_REPORT').first()
    ...
    try:
        ...
        report_data = [
            business_day.date.strftime('%Y-%m-%d'), business_day.location,
            business_day.opening_cash, business_day.total_sales,
            business_day.expected_cash, business_day.closing_cash,
            business_day.cash_diff, business_day.total_transactions,
